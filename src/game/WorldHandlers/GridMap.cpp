...
bool TerrainInfo::Load(const uint32 x, const uint32 y)
{
    ...
    // it already has, which it rejects and logs. Common now that a vessel is an active
    // object holding grids a player then walks into. The tile data itself still loads
    // lazily, on the first query that reaches it.
    if (firstReference)
    {
                                                 ...
                                                 GridMapLiquidData* data) const
{
    ...
    const auto liquid = column.LiquidAt(z);
    if (!liquid || !liquid->liquidEntry)
    {
