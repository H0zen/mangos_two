...
namespace world::terrain
{
    ...
    public:
        ...
        /// attribute: the MCLQ per-cell dark bit, or a cell whose MH2O chunk carries the
        /// deep attribute, both of which the tile's own liquidDeep grid records. MLIQ has
        /// no equivalent, its tile-flag nibble says only which tiles are dry, and vmap
        /// never produced fatigue from WMO liquid either. Adding one here means inventing
        /// the source, so LocalLiquid::deep stays false on this path.
        struct Liquid
        {
